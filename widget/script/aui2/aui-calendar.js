...
(function(window){
    ...
    calendar.prototype._init = function(options) {
        ...
        for (var i = startYear; i <= year; i++) {
             var html = buildCalendarData(i,i,startMonth,me);
             $("#calendar").append(html);
        }
        setDateScroll();
        
        setTimeout(function(){
        	     for(var i=parseInt(year)+1 ;i <= endYear;i++){
        	     	var html = buildCalendarData(i,startYear,startMonth,me);
        	     	$(me.tableContainer).append(html);
        	     }
        ...
        function setDateScroll(){
		        ...
		        $('.today').remove();
		        if($('table tr td[date="'+selectDate+'"]').find(".active").length == 0){
