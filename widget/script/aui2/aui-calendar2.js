...
(function(window){
    ...
    calendar.prototype._init = function(options) {
        ...
        var selectDate = this.options.selectDate;
        dbefore        = this.options.afterDateGay;
        // 判断年开始、结束
        ...
        for (var i = startYear; i <= yearLen; i++) {
            ...
            for (var ii = startMonth; ii < 12; ii++) {
                ...
                $(this.tableContainer).append(tableHtml);
        ...
        //设置第一年的日历
        buildCalendarData(oneYear,startYear,startMonth,this);
        if(year == oneYear){
        	     setDateScroll();
        }
        if(yearLen == oneYear){
        	     if(me.options.dateActive){
	            me._dateActive();
	         }
        }
        ...
        if(twoYear != ""){
        	   setTimeout(function(){
        	   	    buildCalendarData(twoYear,startYear,startMonth,me);
        	   	    if(year == twoYear){
        	   	    	     setDateScroll();
        	   	    	}
        	   	    if(yearLen == twoYear){
        ...
        if(threeYear != ""){
        	   setTimeout(function(){
        	   	    buildCalendarData(threeYear,startYear,startMonth,me);
        	   	    if(year == threeYear){
        	   	    	    setDateScroll();
        	   	    }
        	   	    if(yearLen == threeYear){
		        	     if(me.options.dateActive){
			            me._dateActive();
			         }
		        }
        ...
        if(fourYear != ""){
        	   setTimeout(function(){
        	   	    buildCalendarData(fourYear,startYear,startMonth,me);
        	   	    if(year == fourYear){
        	   	    	     setDateScroll(); 
        	   	    }
        	   	    if(yearLen == fourYear){
        ...
        if(fiveYear != ""){
        	   setTimeout(function(){
        	   	    buildCalendarData(fiveYear,startYear,startMonth,me);
        	   	    if(year == fiveYear){
        	   	    	     setDateScroll();
        	   	    }
        	   	    if(yearLen == fiveYear){
        ...
        if(sixYear != ""){
        	   setTimeout(function(){
        	   	    buildCalendarData(sixYear,startYear,startMonth,me);
        	   	    if(year == sixYear){
        	   	    	     setDateScroll();
        	   	    	}
        	   	    if(yearLen == sixYear){
        ...
        if(sevenYear != ""){
        	   setTimeout(function(){
        	   	    buildCalendarData(sevenYear,startYear,startMonth,me);
        	   	    if(year == sevenYear){
        	   	    	     setDateScroll();
        	   	    	}
        	   	    if(yearLen == sevenYear){
		        	     if(me.options.dateActive){
			            me._dateActive();
			         }
		        }
        ...
        //生成日历数据
        function buildCalendarData(i,startYear,startMonth,me){
               ...
               if(startYear != i){
	            ...
	            for (var ii = startMonth; ii < 12; ii++) {
	                if(endYear == i && ii >= endMonth){
                        ...
                        tableHtml += '<th class="aui-calendar-title aui-text-primary" colspan="7">'+i+'年'+_monthArr[ii]+'</th>';
                        tableHtml += '</tr>';
                        tableHtml += '<tr class="aui-text-primary" data-year="'+i+'" data-month="'+ii+'">'+me._createWeek()+'</tr>';
                        tableHtml += '</thead>';
                        tableHtml += '<tbody class="aui-calendar-body">';
                        tableHtml += me._createDay(i,''+(ii+1)+'');
                        tableHtml += '</tbody>';
	                    tableHtml += '</table>';
	                }
	                $(me.tableContainer).append(tableHtml);
	           }
        ...
        function setDateScroll(){
        	        var year     = selectDate.split("-")[0];
		        var month    = selectDate.split("-")[1] - 1;
		        var tableEle = $('table tr[data-year="'+year+'"][data-month="'+month+'"]').parents('table');
		        var height   = tableEle.offset().top;
		        $('body')[0].scrollTop = height;
		        $('.today').remove();
                $('table tr td[date="'+selectDate+'"]')[0].insertAdjacentHTML('beforeend', '<div class="active"></div>');
        }
    }
    // 判断两个日期
    calendar.prototype._isGreaterDate = function(strDateStart, strDateEnd){
            ...
            html += '<tr>';
                for (var col = 0; col < 7; col++) {
                    if(s >= firstDay && d <= monthLen){
                    	    var dateYmd = year+'-'+this._foo(month)+'-'+this._foo(d);
                        if(_d > d & this.options.beforeStartDateClick == false){
                            html += '<td date="'+ dateYmd +'" class="before-day" click-status="'+this.options.beforeStartDateClick+'">';
                        }else{
	                          if(col==0 || col==6){
	                          	  
	                    		  	    if(dbefore==""||this._isGreaterDate(dbefore,dateYmd)){ 
	                                 	html += '<td date="'+ dateYmd +'" class="aui-text-danger">';
	                                }else{ 
	                                    	html += '<td date="'+ dateYmd +'" class="before-day" click-status="'+this.options.beforeStartDateClick+'">';
	                          	    ...
	                          	    if(dbefore==""||this._isGreaterDate(dbefore,dateYmd)){ 
	                          	    	     html += '<td date="'+ dateYmd +'" class="aui-text-primary">';
	                          	    }else{
	                          	    	     html += '<td date="'+ dateYmd +'" class="before-day" click-status="'+this.options.beforeStartDateClick+'">';
