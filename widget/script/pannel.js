...
var Pannel = function(options){
	return new Pannel.prototype.init(options);
};
Pannel.prototype = {
	version: '1.00',
	author: 'jzy',
	construtor: Pannel, //修正构造函数指针
	init: function(options){
		...
		var styleText = [
							...
							'height:' + this.pStyle.height + 'px; ',
							'margin-left:' + (-1 * Math.ceil(this.pStyle.width/2)) + 'px; ',
							'margin-top:' + (-1 * Math.ceil(this.pStyle.height/2)) + 'px; '
						].join('');
		...
		var wraptHTML = [
							'<div class="pannelWrapper">',
								'<div class="pannelBox" style=";' + styleText + '">',
									'<span class="aui-iconfont pannelClose">&#xe659;</span>',
									'<div class="pannelMain" style="height:', iMainHei ,'px;" id="wrapper">',
	...
	execu: function(str){
		...
		//给右上角关闭按钮绑定关闭事件
		$('.pannelClose').on('tap', function(ev){
			//阻止冒泡
			ev.stopPropagation();
...
Pannel.prototype.init.prototype = Pannel.prototype;
